...
class Solution {
  public:
    Node* dfs(Node *node, unordered_map<Node*,bool> &vis, unordered_map<Node*,Node*> &alternate){
        if(vis.count(node))
            return alternate[node];
        vis[node]=true;
        Node *vertex=new Node(node->val);
        alternate[node]=vertex;
        for(auto neigh : node->neighbors){
            Node *child = dfs(neigh,vis,alternate);
            vertex->neighbors.push_back(child);
        }
        return vertex;
    }
    Node* cloneGraph(Node* node) {
        ...
        unordered_map<Node*,Node*> alternate;
        return dfs(node, vis, alternate);
    }
};
